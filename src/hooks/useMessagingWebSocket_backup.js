...
export const useMessagingWebSocket = (conversationId, onMessage) => {
  ...
  const sendMessage = useCallback((content) => {
    if (ws.current && ws.current.readyState === WebSocket.OPEN) {
      ...
      console.log('[WebSocketHook] Sending message via WebSocket:', message);
      ws.current.send(JSON.stringify(message));
      return true;
    } else {
      console.warn('[WebSocketHook] Cannot send message: WebSocket not connected');
      return false;
    }
  }, []);
  ...
  useEffect(() => {
    const handleOnline = () => {
      console.log('[WebSocketHook] Network back online, attempting to reconnect');
    ...
    const handleOffline = () => {
      console.log('[WebSocketHook] Network went offline');
